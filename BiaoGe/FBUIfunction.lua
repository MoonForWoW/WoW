...
function BG.FBDiSsUI(FB,t,b,bb,i,ii)
    ...
    ds:Show()
    TextureDs[FB..1]= ds:CreateTexture()
    TextureDs[FB..1]:SetAllPoints(ds)
    ds:SetNormalTexture(TextureDs[FB..1])
    BG.FrameDs[FB..1]["boss"..BossNum(FB,b,t)]["ds"..i] = TextureDs[FB..1]
        -- 底色材质2（点击框体后）
    local ds = CreateFrame("Button", nil, BG["Frame"..FB])
    ds:SetSize(331, 18)
    ...
    ds:Show()
    TextureDs[FB..2]= ds:CreateTexture()
    TextureDs[FB..2]:SetAllPoints(ds)
    ds:SetNormalTexture(TextureDs[FB..2])
    BG.FrameDs[FB..2]["boss"..BossNum(FB,b,t)]["ds"..i] = TextureDs[FB..2]
    ...
    ds:Show()
    TextureDs[FB..3]= ds:CreateTexture()
    TextureDs[FB..3]:SetAllPoints(ds)
    ds:SetNormalTexture(TextureDs[FB..3])
    BG.FrameDs[FB..3]["boss"..BossNum(FB,b,t)]["ds"..i] = TextureDs[FB..3]
end
...
function BG.FBZhuangBeiUI(FB,t,b,bb,i,ii)
    ...
    button:SetScript("OnEnter", function(self) 
        BG.FrameDs[FB..1]["boss"..BossNum(FB,b,t)]["ds"..i]:SetColorTexture(red,greed,blue,touming1)
        -- if BG.paimaimsgFrame:IsVisible()then
        --     return
        -- end
        if not tonumber(self:GetText()) then
    ...
    button:SetScript("OnLeave", function(self) 
        BG.FrameDs[FB..1]["boss"..BossNum(FB,b,t)]["ds"..i]:SetColorTexture(0,0,0,0)
        GameTooltip:Hide() 
    end)
        -- 获得光标时
    button:SetScript("OnEditFocusGained", function(self)
        ...
        BG.FrameDs[FB..2]["boss"..BossNum(FB,b,t)]["ds"..i]:SetColorTexture(red,greed,blue,touming2)    -- 底色
    end)
        -- 失去光标时
    button:SetScript("OnEditFocusLost", function(self)
        ...
        BG.FrameDs[FB..2]["boss"..BossNum(FB,b,t)]["ds"..i]:SetColorTexture(0,0,0,0)
    end)
        -- 按TAB跳转右边
...
function BG.FBJiShaUI(FB,t,b,bb,i,ii)
    if BG.Loot.encounterID[FB][BossNum(FB,b,t)] then
        ...
        local bossname = BiaoGe[FB]["boss"..BossNum(FB,b,t)]["bossname"]
        ...
        if BiaoGe[FB]["boss"..BossNum(FB,b,t)]["time"] then
            if BiaoGe[FB]["boss"..BossNum(FB,b,t)]["time"] ~= "" then
                if color then
                    text:SetText(color.."击杀用时："..BiaoGe[FB]["boss"..BossNum(FB,b,t)]["time"])
                else
                    text:SetText("击杀用时："..BiaoGe[FB]["boss"..BossNum(FB,b,t)]["time"])
                end
            end
        end
    end
